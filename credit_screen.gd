...
@onready var logo_node = $CanvasLayer2/LogoTextureRect
...
@onready var next_scene_path = "res://StartScreen/start_screen.tscn"
...
func _change_scene(path: String):
	...
	logo_node.modulate = initial_color
	...
	tween.parallel().tween_property(text_node, "modulate", target_transparent_color, transition_duration).set_ease(Tween.EASE_IN)
	tween.parallel().tween_property(final_prompt_label, "modulate", target_transparent_color, transition_duration).set_ease(Tween.EASE_IN)
	tween.parallel().tween_property(logo_node, "modulate", target_transparent_color, transition_duration).set_ease(Tween.EASE_IN)
	tween.parallel().tween_property(background_music, "volume_db", -60.0, transition_duration).set_ease(Tween.EASE_IN)
